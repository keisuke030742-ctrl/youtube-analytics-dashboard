...
それは、「断熱性能が高い家」であること。
断熱スカスカの家でつけっぱなしにしたら、
入れた熱が全部逃げていくので、ただの電気代の垂れ流しになります。

なので、本気で光熱費を下げたいなら、
「エアコンの使い方」を気にする前に、
「熱が逃げない家」かどうかを気にしてください。
性能さえあれば、エアコン1台で家中快適、電気代も半分以下。
これが2026年の常識ですよ。
\`\`\`
...
つまり、坪単価で比較するのって、何の意味もないんです。
お店に入って「この服いくらですか？」って聞いてるのに、
「布代だけなら安いです」って言われてるようなもんですよ。
...
これは多くの人が「老後も安心」と思って選びますが、実はランニングコストが原因で、老後破綻をして住み続けたマイホームを手放さなければいけなくなります。

たとえば夏、屋根からの熱が直接生活空間に降り注ぎ、エアコンをフル稼働しても効かない。
冬は床下からの冷気で足元が凍える。
2階建てより断熱面積が大きく、光熱費が1.5倍になるケースも…。

それが――断熱をケチった平屋で終わりなき光熱費地獄
...
### ステップ1: 分析（必ず実行）
まず以下を簡潔に分析してから企画を作成すること：

1. **このテーマで視聴者が本当に知りたいこと**は何か？（3つ）
2. **競合が見落としている切り口**は？（2つ）
3. **平松社長だから言える独自の視点**は？（2つ）
4. **使えそうなパワーワード**を5つ挙げる

### ステップ2: 企画作成
分析を踏まえて、以下の5パターンで企画を作成：
- **企画A**: 逆張り・常識破壊型（「実は〇〇は間違い」）
- **企画B**: TOP3/ランキング型（「知らないと損するTOP3」）
- **企画C**: 損失回避・警告型（「これをやると〇〇万円損」）
- **企画D**: 意外な事実・暴露型（「業界が隠している真実」）
- **企画E**: 比較・対決型（「AとBどっちが得？」）
...
### 出力形式
1. まず「## 分析」として上記ステップ1の分析結果を簡潔に出力
2. その後「## 企画案」として以下のJSON形式で出力：

\`\`\`json
{"proposals": [{"rank": 1, "title": "...", "hook": "...", "concept": "...", "targetEmotion": "...", "estimatedViews": "...", "viralScore": 85, "reasoning": "...", "script": "...", "suggestedKeywords": ["キーワード1", "キーワード2", "キーワード3"]}, ...]}
\`\`\``;
}
