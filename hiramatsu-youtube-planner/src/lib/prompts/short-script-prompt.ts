...
上記のワークフローに従って**内部で分析**を行い、**5つの異なる切り口の企画案**を作成してください。

### 重要な制約
1. 5つの案は**それぞれ異なるアプローチ**で作成すること（例：逆張り、TOP3、損失回避、意外な事実、比較など）
2. 必ずNGワードルールを遵守すること
3. 各台本は60秒以内で読める分量（400〜600文字程度）に収めること
4. 再生回数が伸びると予測される順にランキングすること
5. **年号を含める場合は必ず現在の年（${new Date().getFullYear()}年）を使用すること**

### 出力形式（厳守）
- **ワークフローの過程は出力しないでください**
- **最終結果のJSONのみを出力してください**
- 説明文やマークダウンは一切不要です
- 以下の形式で、JSONのみを返してください：

{"proposals": [{"rank": 1, "title": "...", "hook": "...", "concept": "...", "targetEmotion": "...", "estimatedViews": "...", "viralScore": 85, "reasoning": "...", "script": "..."}, ...]}`;
}
